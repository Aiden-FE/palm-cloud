...
export async function main(): Promise<1 | -1> {
  ...
  const [result] = await connection.query(`SELECT id, name FROM migrations WHERE name = 'resources' LIMIT 1`);

  if (result[0]) {
    await connection.end();
    Logger.log(`已执行过 resources 的迁移任务`);
    return -1;
  } else {
    await connection.query(readFileSync(join(__dirname, './main.sql'), { encoding: 'utf-8' }));
    await connection.query(`INSERT INTO migrations (name) VALUES ('resources')`);
    await connection.end();
    Logger.log(`resources 的迁移任务执行完成`);
    return 1;
  }
}
...
export async function rollback(): Promise<1 | -1> {
  ...
  if (result[0]) {
    ...
    await connection.query("DELETE FROM migrations WHERE `name` = 'resources'");
    await connection.end();
    Logger.log(`resources 的回滚任务执行完成`);
    return 1;
  } else {
    await connection.end();
    Logger.log(`resources 的迁移任务已回滚或不存在`);
    return -1;
  }
}
